        ...
        async with self._condition:
            while True:
                ...
                if self._current_size < self.pool_size:
                    ...
                    # Release lock while creating connection to allow others to proceed
                    ...
                    try:
                        ...
                        # Reacquire lock before returning is not strictly necessary here
                        # as we are not modifying shared state after creation.
                        return wrapper
                    except Exception as e:
                        # Failed to create, decrement size and re-acquire lock to check again/wait
                        logger.error(f"Failed to create connection for pool: {e}")
                        await self._condition.acquire()
                        self._current_size -= 1
                        # Need to decide whether to retry immediately or let the loop handle it
                        # For now, let the loop check timeout and potentially wait
                    finally:
                        # Ensure lock is re-acquired if released and connection creation failed
                        if not self._condition.locked():
                             await self._condition.acquire()


                # If pool is full and no connection available, wait
