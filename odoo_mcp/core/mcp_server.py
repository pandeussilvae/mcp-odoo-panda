...
class MCPServer:
    ...
    async def handle_request(self, request_data: Dict) -> Dict:
        ...
        validated_dict = None
        try:
            # validate_request_data now returns a dictionary
            validated_dict = validate_request_data(request_data)
            # Access fields using dictionary keys
            method_name = validated_dict["method"]
            params = validated_dict["params"] # This is now EchoParams, CallOdooParams, or dict
            request_id = validated_dict["id"] # Use validated ID

            # Get session_id from params if it's a model or a dict
            if isinstance(params, BaseModel): # Check if Pydantic model (like EchoParams)
            ...
            return {"jsonrpc": "2.0", "error": {"code": -32602, "message": error_msg}, "id": request_id}
             ...
             # Ensure request_id is used even if validation fails early
             return {"jsonrpc": "2.0", "error": {"code": -32600, "message": f"Invalid Request: {e}"}, "id": request_data.get("id")}
        ...
        try:
            # Use validated method_name and params (which is EchoParams, CallOdooParams, or dict)
            logger.info(f"Dispatching method: {method_name}")

            if method_name == "echo":
                 # Check if params is the expected Pydantic model or fallback dict
                 if isinstance(params, EchoParams):
                      message_to_echo = params.message
                 elif isinstance(params, dict): # Fallback if Pydantic wasn't available/failed subtly
                      message_to_echo = params.get("message", "Default echo (dict fallback)")
                 else:
                      raise ProtocolError(f"Internal error: Invalid params type for echo: {type(params)}")
                 response["result"] = message_to_echo

            elif method_name == "call_odoo":
                 # Check if params is the expected Pydantic model or fallback dict
                 if isinstance(params, CallOdooParams):
                      model = params.model
                      odoo_method = params.odoo_method
                      args = params.args
                      kwargs = params.kwargs
                      service = params.service
                 elif isinstance(params, dict): # Fallback if Pydantic wasn't available/failed subtly
                      logger.warning("call_odoo received params as dict, attempting manual extraction.")
                      model = params.get("model")
                      odoo_method = params.get("method") # Use original 'method' key if alias didn't happen
                      args = params.get("args", [])
                      kwargs = params.get("kwargs", {})
                      service = params.get("service")
                      if not model or not odoo_method:
                           raise ProtocolError("Missing 'model' or 'method' in params for call_odoo (dict fallback)")
                 else:
                      raise ProtocolError(f"Internal error: Invalid params type for call_odoo: {type(params)}")
